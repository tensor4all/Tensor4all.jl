...
All user-facing APIs use **1-indexed** indices (Julia convention).
Internal C API calls convert to 0-indexed.
...
export bonddims, maxbonderror, maxrank, maxsamplevalue
export to_treetn
...
function _infer_scalar_type(f, local_dims::Vector{<:Integer}, initial_pivots::Vector{Vector{Int}})
    sample_indices = isempty(initial_pivots) ? ones(Int, length(local_dims)) : initial_pivots[1]
...
# The Rust side passes 0-indexed batch values. The trampoline adds 1 before
# calling the user function so that user functions always see 1-indexed values.

"""
Internal trampoline for f64 batch callbacks.

The user function signature is: `f(batch::Matrix{Int}) -> Vector{Float64}`
where `batch` is column-major `(n_sites, n_points)` with **1-based** indices.

The C API provides 0-based indices; we add 1 before calling the user function.
"""
function _treetci_batch_trampoline(
    ...
    try
        ...
        f = f_ref[]
        raw_batch = unsafe_wrap(Array, batch_data, (Int(n_sites), Int(n_points)))
        # Convert 0-indexed (from C API) to 1-indexed (for user)
        batch_1indexed = Int.(raw_batch) .+ 1
        vals = f(batch_1indexed)
...
The user function signature is: `f(batch::Matrix{Int}) -> Vector{ComplexF64}`
where `batch` is column-major `(n_sites, n_points)` with **1-based** indices.
...
function _treetci_batch_trampoline_c64(
    ...
    try
        ...
        # Convert 0-indexed (from C API) to 1-indexed (for user)
        batch_1indexed = Int.(raw_batch) .+ 1
        vals = ComplexF64.(f(batch_1indexed))
        length(vals) == Int(n_points) ||
            error("Batch callback returned $(length(vals)) values for $(Int(n_points)) points")
...
add_global_pivots!(tci, [ones(Int, 4)])
for _ in 1:20
    sweep!(tci, f; tolerance=1e-8)
    maxbonderror(tci) < 1e-8 && break
end
ttn = to_treetn(tci, f)
```
"""
...
Add global pivots. Each pivot is a full multi-index over all sites (**1-based**).
Internally converts to 0-based for the C API.

# Arguments
- `tci::SimpleTreeTci`
- `pivots::Vector{Vector{Int}}`: Each element has length `n_sites`, 1-based indices
"""
function add_global_pivots!(tci::SimpleTreeTci{T}, pivots::Vector{Vector{Int}}) where {T}
    ...
    for j in 1:n_pivots
        ...
        for i in 1:n_sites
            # Convert 1-indexed to 0-indexed for C API
            pivots_flat[i + n_sites * (j - 1)] = Csize_t(pivot[i] - 1)
        end
    end
...
# Arguments
- `tci::SimpleTreeTci`
- `f`: Batch evaluation function `f(batch::Matrix{Int}) -> Vector{T}`
  where `batch` is column-major `(n_sites, n_points)` with **1-based** indices
...
function maxbonderror(tci::SimpleTreeTci{T}) where {T}
    out = Ref{Cdouble}(0.0)
    C_API.check_status(ccall(
...
function maxrank(tci::SimpleTreeTci{T}) where {T}
    out = Ref{Csize_t}(0)
    C_API.check_status(ccall(
        _sym_for(T, :max_rank),
        Cint,
...
function maxsamplevalue(tci::SimpleTreeTci{T}) where {T}
    out = Ref{Cdouble}(0.0)
    C_API.check_status(ccall(
...
function bonddims(tci::SimpleTreeTci{T}) where {T}
...
- `f`: Batch evaluation function (same as `sweep!`; receives 1-indexed batches)
...
Evaluate a TreeTensorNetwork at a single multi-index (**1-based**).
Returns a scalar (`Float64` or `ComplexF64` depending on storage).

# Example
```julia
val = evaluate(ttn, [1, 2, 3, 1])
```
"""
function evaluate(ttn::TreeTensorNetwork, indices::Vector{<:Integer})
    # Convert 1-indexed to 0-indexed for C API
    indices_0 = Csize_t.(indices .- 1)
    vals = _evaluate_batch(ttn, indices_0, length(indices), 1)
    return vals[1]
end

"""
    evaluate(ttn, indices::Vector{Vector{T}}) -> Vector

Evaluate a TreeTensorNetwork at multiple multi-indices (**1-based**).
Each element of `indices` is a multi-index of length `n_sites`.

# Example
```julia
vals = evaluate(ttn, [[1,2,3,1], [2,1,2,2]])
```
"""
function evaluate(ttn::TreeTensorNetwork, indices::Vector{Vector{T}}) where {T<:Integer}
    ...
    for j in 1:n_points
        ...
        for i in 1:n_sites
            # Convert 1-indexed to 0-indexed for C API
            flat[i + n_sites * (j - 1)] = Csize_t(indices[j][i] - 1)
        end
    end
...
# Example
```julia
batch = [1 2; 2 1; 3 2; 1 2]  # 4 sites, 2 points, 1-based
...
function _evaluate_batch(ttn::TreeTensorNetwork, flat::Vector{Csize_t}, n_sites::Int, n_points::Int)
    ...
    # flat values are already 0-indexed (caller does the conversion).
    ...
    for p in 0:(n_points - 1)
        for i in 1:n_indices
            ...
            reordered[i + n_indices * p] = flat[site + 1 + n_sites * p]  # +1 for Julia 1-based array
        end
    end
...
- `f`: Batch evaluation function -- `f(batch) -> Vector{T}` where `batch` is a
  `Matrix{Int}` of shape `(n_sites, n_points)` in column-major layout.
  `batch[i, j]` is the **1-based** local index at site `i` for evaluation point `j`.
...
# Keyword Arguments
- `initialpivots::Vector{Vector{Int}} = [ones(Int, n)]`: Initial pivots (**1-based**)
...
# Batch evaluation function (1-based indices)
function f(batch)
...
function crossinterpolate2(
    ...
    graph::TreeTciGraph;
    initialpivots::Vector{Vector{Int}} = [ones(Int, graph.n_sites)],
    tolerance::Float64 = 1e-8,
    ...
    # Create state and add initial pivots (1-indexed; add_global_pivots! converts to 0-indexed)
    ...
    for iter in 1:maxiter
        ...
        r = maxrank(tci)
        err = maxbonderror(tci)
        msv = maxsamplevalue(tci)
